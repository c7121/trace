...
Configure this as a source DAG job using `operator: liveliness_monitor` with `source.kind: cron`. See [liveliness_monitor](../architecture/operators/liveliness_monitor.md#example-dag-config) for an example job entry, and [dag_configuration.md](../specs/dag_configuration.md) for the full job YAML schema.
