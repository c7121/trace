...
Lower-level component details live in each container document under [containers/](containers/README.md) (see the L3 index below).

For AWS deployment details and Terraform structure, see [../deploy/infrastructure.md](../deploy/infrastructure.md).
...
- **Buffered datasets** are implemented by platform worker sink consumers ([ADR 0006](../adr/0006-buffered-postgres-datasets.md)).
- **Workers have no direct internet egress.** Any external calls happen through dedicated egress services: [Delivery Service](containers/delivery_service.md) and [RPC Egress Gateway](containers/rpc_egress_gateway.md). See [ADR 0002](../adr/0002-networking.md).
- **Lambda UDF runner does not connect to Postgres directly.** It reads via Query Service and obtains scoped S3 credentials via Dispatcher credential minting. See [Workers](containers/workers.md) and [credential_minting.md](contracts/credential_minting.md).
- The Dispatcher is the orchestration control plane; Postgres state is the durable source of truth; the **Task Queue** is a wake-up mechanism (SQS in AWS, pgqueue in Lite). See [task_lifecycle.md](task_lifecycle.md).
