...
flowchart TB
    ...
    udf_workers -->|request creds| broker
    udf_workers -->|read and write parquet| s3
    udf_workers -->|publish records| buffer_sqs

    broker -->|assume role| sts
    broker -->|temp creds| udf_workers

    buffer_sqs -->|drain buffered datasets| sinks
    sinks -->|write buffered datasets| pg_data
